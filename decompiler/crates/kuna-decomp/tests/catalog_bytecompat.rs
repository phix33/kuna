...
#[test]
fn fixture_has_all_156_settables() {
    ...
    assert_eq!(FIXTURE.matches("\"option\": ").count(), 156);
    // Every row carries the tier field appended after change_kind.
    assert_eq!(FIXTURE.matches("\"tier\": ").count(), 156);
    // ... and the symptoms array appended after tier (C3).
    assert_eq!(FIXTURE.matches("\"symptoms\": ").count(), 156);
}
